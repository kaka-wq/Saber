...
int main()
{
	...
	int *array3;
    array3=(int*)calloc(128,sizeof(int));
	//int array3[2048]={0};//find out the differnt bit strings and record the number of each bit string in array3
	int *array4;
    array4=(int*)calloc(128,sizeof(int));
	//int array4[2048]={0};//record the differnt bit strings in array4
	int count=0;//the number of the differnt bit strings
	int array5[7][10]={0};//the row is the possible values of S_t and the column is the number of the differnt bit strings
	int array6[7][4]={0};//choose 5 columns  from this array5, so that each row of the selected matrix is different
	int column=4;////choose 5 columns  from this array5
...
*/
				...
				cout<<"in order to output the first result, as shown above,"<<endl;
				for (b = 0; b<p; b++)
				{
					for (a = 0; a<t; a++)
					{
