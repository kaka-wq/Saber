...
In other words, We need to find as few columns as possible from a 9-by-2^{(d_u + d_v)} matrix so that each
row of the selected matrix is different. There are at most 2^9 different columns in the matrix and many (a,b) 
...
and we will choose as few columns as possible from this small matrix. Since s_(i,j) has 9 possible values, 
at least 4 columns are needed. However, by searching, we found that there are no 4 columns that meet the 
conditions, then we try to choose 5 columns.
*/
#include<iostream>
#include <stdio.h>
#include <math.h>
...
int main()
{
	...
	int et=4;
	int t=16;//2^et
	int Mpt=64;//Mpt=2^(ep-et)=2^6
	int h2=228;//h2=256-2^(ep-et-1)+4
	int eta=4;
	int pv=9;//2*eta+1 possible values of S_t
	int mpv=512;//2^pv
	int array1[9]={0};//record m_j correspongding to S_t,j!=0
	int array2[9]={0};//recover m_0 correspongding to S_t
	int *array3;
    array3=(int*)calloc(512,sizeof(int));//2^pv,find out the differnt bit strings and record how many (a,b) paris correspond to each bit string in array3
	int *array4;
    array4=(int*)calloc(512,sizeof(int));//2^pv,record the differnt bit strings in array4 and transform the bit string into integer and record it in array4
	int count=0;//the number of the differnt bit strings,count=13
	int array5[9][13]={0};//9 is the possible values of S_t and 13 is the number of the differnt bit strings
	int array6[9][5]={0};//choose 5 columns  from this array5, so that each row of the selected matrix is different
	int column=5;////choose 5 columns  from this array5
	...
	for (b = 0; b<p; b++)
	{
		...
		{
			flag=true;
			for(st=0;st<pv;st++)////S_t in {-4,-3,...,3,4}
			{
		   ...
		   //for a (a, b) pair, m_0's values constitutes a 9-bit string when S_t takes 9 different values
		   //since the length of the bit string is 9, it has 2^9 possibles and a large number of (a, b) pairs corresponds to the same bit string 
           //convert the bit strings into integers for the convenience of finding the same bit string 
			sum=0;
			mula=1;
			for(st=0;st<pv;st++)//S_t has 9 possible values and the length of the bit string is 9 
			{
	...
	j=0;
	for(i=0;i<mpv;i++)//since the length of the bit string is 9, it has 2^9 possibles
	{
		if (array3[i]!=0)
		{
			array4[j]=i;
	...
	for (j=0;j<count;j++)//count=13
	{
		temp=array4[j];
		array5[0][j]=temp%2;
		for(i=1;i<pv;i++)//9 possible values
		{
			temp=temp/2;
			array5[i][j]=temp%2;
		}
	...
	for (j=0;j<count;j++)//count=13
	{
		...
		for(i=0;i<pv;i++)//9 possible values
		{
			cout<<array5[i][j];
		}
		cout<<endl;
   ...
   //Remove a large number of duplicates and we get a 9-by-count matrix.
	//This matrix is much smaller than the previous 9-by-(d_v*d_u) matrix
   //We choose as few columns as possible from this matrix, so that each row of the selected matrix is different 
	//Since S_t has 9 possible values, at least 4 columns are needed ,but there are no 4 columns that meet the conditions
	//In particualr, we choose 5 columns
	for (i=0;i<count;i++)//count=13
		for(j=i+1;j<count;j++)//count=13
			for(k=j+1;k<count;k++)//count=13	
				for(l=k+1;l<count;l++)//count=13	
					for(m=l+1;m<count;m++)//count=13	
						
				{
					flag=true;
					for(tt=0;tt<pv;tt++)//9 possible values
					{
					...
					f=pv-1;//9 possible values
					for(d=0;d<pv;d++)//9 possible values
					{	
						for(e=1;e<=f;e++)
						{
							if(array6[d][0]==array6[d+e][0] && array6[d][1]==array6[d+e][1] && array6[d][2]==array6[d+e][2] && array6[d][3]==array6[d+e][3] && array6[d][4]==array6[d+e][4])
							{
								flag=false;
								break;
							}
					...
					if(flag)
					{
						...
						//{
						    //since there are 3 choices and we output all the choices
						    cout<<"We choose 5 columns so that each row of the selected matrix is different"<<endl;
							cout<<"the  result is: "<<endl;
							controloutput++;
							for(g=0;g<pv;g++)//9 possible values
							{
								for(h=0;h<column;h++)//try to distinguish each coefficient of s using 5 queries
								{
									cout<<array6[g][h];
								}
								cout<<endl;
							...
							cout<<"they are taken from columns"<<" "<<i<<" "<<j<<" "<<k<<" "<<l<<" "<<m<<" of the  array5"<<endl;
							//cout<<"i="<<i<<", j="<<j<<", k="<<k<<", l="<<l<<endl;
							cout<<" "<<endl;

							//record the 5 columns in order to find the corresponding (a,b) pair next 
							i_output1=i;
							j_output1=j;
							k_output1=k;
							l_output1=l;
...
*/
				...
				cout<<"there are "<<number<<" results, as shown above, ";
				cout<<"in order to output the last result,"<<endl;
				for (b = 0; b<p; b++)
				{
					for (a = 0; a<t; a++)
					{
						...
						sum=0;
						mula=1;
						for(st=0;st<pv;st++)//9 possible values
						{
							sum=sum+array2[st]*mula;
